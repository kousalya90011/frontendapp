...
export default function ChangeSellerPwd() {


    const [sellerData, setsellerData] = useState("");

    useEffect(() => {
      const storedSellerData = localStorage.getItem('admin');
      if (storedSellerData) {
  ...
  const handleSubmit = async (e) => {
    ...
    try 
    {
      const response = await axios.put(`${config.url}/changesellerpwd`, {...formData,"username":sellerData.username});
